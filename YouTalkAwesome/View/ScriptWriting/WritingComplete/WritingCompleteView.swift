...
let structureSectionSample: [Section] = [
    Section(topContent: "Point (요점)", bodyContent: "AI는 UX디자이너의 작업을 혁신적으로 변화시키고, 더 효율적으로 창의적인 작업 환경을 제공합니다.", isScript: true),
    Section(topContent: "Reason (이유)", bodyContent: "티몬과 위메프가 기업회생절차를 신청한 건, 수천억원대에 이르는 셀러(판매자) 미정산 대금 등 회사의 빚을 온전히 감당할 수 없기 때문으로 풀이된다. 티몬과 위메프를 지배하는 구영배 큐텐 대표가 보유 지분 매각 등을 통한 사재 출연을 약속했으나, 이마저도 채무 상환엔 턱없이 부족하다는 의미다.", isScript: true),
    Section(topContent: "🫲", bodyContent: "\"혁신적인\"에서 제스처", isScript: false),
    Section(topContent: "Example (예시)", bodyContent: "통상 법원은 기업의 신청일로부터 1개월 안에 회생절차 개시 여부를 결정한다. 티몬과 위메프의 경우 자율 구조조정 지원 프로그램(ARS프로그램)을 신청한 까닭에 채권자들의 견해와 법원 판단에 따라서 최장 3개월간 회사와 채무자 간 자율 협의 절차를 거칠 수도 있다. 여기서 빚 상환 여부 등이 합의되면 회생절차를 취소하고, 협약 체결이 무산되면 회생절차를 진행하게 된다.", isScript: true),
    Section(topContent: "🙂", bodyContent: "미소짓기", isScript: false),
    Section(topContent: "Point (요점)", bodyContent: "두 회사의 회생 절차 신청과 셀러·피지사들의 손실 복구 장기화 등으로 뒤늦게 지원책과 제도 보완에 나선 정부 책임론도 커질 것으로 보인다. 금융 당국은 이날 “회사의 판매 대금 미정산으로 이미 피해가 현실화된 만큼 회생 신청으로 인해 상황이 크게 달라지지 않을 것”이라며 “기존에 마련한 정부 지원책을 신속하게 집행하고 필요하면 추가적인 지원 방안을 모색할 것”이라고 했다.", isScript: true),
    Section(topContent: "⭐️", bodyContent: "마지막 문장 강조", isScript: false)
]
